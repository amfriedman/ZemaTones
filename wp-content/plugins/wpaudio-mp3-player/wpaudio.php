...
function wpaLibraries(){
	...
	if ( version_compare( get_bloginfo( 'version' ), '2.8', '>=' ) ) {
		if ( WP_DEBUG === false ) {
			wp_register_script( 'wpaudio', WPAUDIO_URL . '/wpaudio.min.js', Array('jquery'), $wpa_version, true ); // wpaudio.min.js
		}
		else {
...
function wpaLink($url, $text = false, $dl = true, $autoplay = false) {
	...
	### AMF Modifications 1/15/2012
	$arr = explode('/wp-content/',$url);
	$key = md5(substr($url,-10));
	$url = 'http://zematones.com/wp-content/themes/zema/get_file.php?f='.urlencode($arr[count($arr)-1]).'&t='.time().'&k='.$key;
