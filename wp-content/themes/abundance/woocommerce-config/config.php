...
function avia_woocommerce_thumbnail()
{
	...
	### AMF Modifications 1/14/2012
	# This modification is dependent on the WPAudio plugin, which uses the Soundmanager2 library
	# to generate cross-platform/browser audio players
	...
	// If mobile detected, then use the simple HTML5 player
	foreach(array('android','iphone','ipad') as $mobile_device) {
		if(strpos(strtolower($_SERVER['HTTP_USER_AGENT']),$mobile_device) !== false) {
			$is_mobile = true;
			break;
		}
	}
	
	// TESTING: pretend all is mobile
	// $is_mobile = true;
	...
	if ($attachments) {
		...
		foreach ($attachments as $a) {
			$preview_file_types = array('m4r'); // add mp3?
			$ext = strtolower(substr($a->guid,-3));
			
			if(in_array($ext,$preview_file_types)) {
				$is_audio = true;
				$attachment_id = $a->ID;
				$product_file_path = get_attached_file($attachment_id); // on file system
				$product_file_url  = wp_get_attachment_url($attachment_id); // http
				break; 
			}
		}
	}
	
	if($is_audio && function_exists('wpaLink')) {
		
		#echo 'wpa: '.$product_file_url;
		 
		$text = false;
		$dl   = false;
		$autoplay = false;
	
		$wpaudio_embed = wpaLink($product_file_url, $text, $dl, $autoplay);
		
		$wpaudio_embed .= $is_mobile? '<audio src="'.$product_file_url.'" id="html5player_'.$attachment_id.'" class="html5_player" preload="auto" controls="controls"></audio>': '';
		
		// $wpaudio_embed = '<a href="#" >Test Link</a>'; 
	}
	...
	echo "<div class='thumbnail_container_inner'>";
		...
		if(!$img) {
			echo '<img src="http://zematones.com/wp-content/uploads/2012/01/speaker_light.jpg" class="attachment-shop_catalog wp-post-image" alt="Ringtone" title="Ringtone" style="visibility: visible; display: block; opacity: 1; ">';	
		} else { echo $img; }
		...
		// If this is a normal product, then display a hard link to the product page
		if(!$is_audio) {
			echo "<a class='button show_details_button $extraClass' href='".get_permalink($post->ID)."'>".__('Show Details','avia_framework')."</a>";
		} 
		// Elseif this is an audio product (eg, ringtone), then display a Play button, and no link to the page
		else { 
			echo "<a onclick='toggle_play_$attachment_id(\"".($is_mobile? "mobile":"")."\");' id='play_$attachment_id' class='button show_details_button play_button $extraClass' href='javascript:void(0);'>".__('Play','avia_framework')."</a>";
			
		 
			echo "<div id='wpaudioplayer_$attachment_id' class='wpaudioplayer' style=' visibility:hidden; height: 1px; width: 1px; overflow: hidden;'>$wpaudio_embed</div>";
			
			echo "<script type='text/javascript'>
					/**
					* @param string type = 'mobile' or undefined
					**/
					function toggle_play_$attachment_id(type) {
						if(typeof type == 'undefined') { var type = null; }
						
						var wpa = jQuery('#wpaudioplayer_$attachment_id').find('span > a');
						var wpa_all = jQuery('.wpaudioplayer').find('span > a');
						
						// Mobile flow: click the simple HTML5 audio tag, 
						// and artificially swap token element classes.
						if(type == 'mobile') { 
							jQuery('.html5_player').each(function(k,v){ 
								v.pause(); 
								v.currentTime = 0; // rewind 
							});
							
							// Pause if already playing
							if(wpa.hasClass('wpaudio-playing')) {
								wpa.removeClass('wpaudio-playing');
							} 
							// Else play it.
							else {
								 
								// Play the HTML5 audio element (must use get(0).play()...)
								jQuery('#wpaudioplayer_$attachment_id audio').get(0).play();
								
								wpa_all.removeClass('wpaudio-playing');
								
								wpa.addClass('wpaudio-playing');
							}
						} 
						// Normal flow
						else {
						...
						if(wpa.hasClass('wpaudio-playing')) {
							jQuery('.button.play_button').html('Play');
							jQuery('#play_$attachment_id').html('Stop Playing');	
						} else {
							jQuery('#play_$attachment_id').html('Play');	
						}
